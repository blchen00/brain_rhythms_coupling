...
% 07/21/20: Bolun Chen
%
% updates on 08/19/20: plotted C3 barplots for subject 902
%
clear; clc; close all
%% Basic Settings & Parameters
% set input & output paths
input_path = '../data/correlations/xcorr_spnorm/';       % where input .mat locates
subject_input_path = '../data/correlations/pairwise_xcorr/C3/subjects/';    % pariwise xcorr including test 1
output_path = '../raw_figs/subject_902/';     % where to save figures
...
neg_corr_table = array2table(squeeze(corr_ratios_grp(1,:,:))', ...
                             'VariableNames', {pairlabel{:}}, ...
                             'RowNames',{statelabel{:}}); 
pos_corr_table = array2table(squeeze(corr_ratios_grp(2,:,:))', ...
                             'VariableNames', {pairlabel{:}}, ...
                             'RowNames',{statelabel{:}}); 
neg_corr_std_table = array2table(squeeze(corr_ratios_grp(3,:,:))', ...
                             'VariableNames', {pairlabel{:}}, ...
                             'RowNames',{statelabel{:}}); 
pos_corr_std_table = array2table(squeeze(corr_ratios_grp(4,:,:))', ...
                             'VariableNames', {pairlabel{:}}, ...
                             'RowNames',{statelabel{:}}); 
% save these tables as Excel spreadsheets
exel_loc = '../graph_data_excel_sheets/';
excel_name = 'fig_3a.xlsx';
file_name = strcat(exel_loc,excel_name);

writetable(neg_corr_table, file_name, 'Sheet','neg_corr','WriteRowNames',true)
writetable(pos_corr_table, file_name, 'Sheet','pos_corr','WriteRowNames',true)
writetable(neg_corr_std_table, file_name, 'Sheet','neg_corr_std','WriteRowNames',true)
writetable(pos_corr_std_table, file_name, 'Sheet','pos_corr_std','WriteRowNames',true)
...
save_dir = '../../Stat_Tests_Wilcoxon_barchats/';                        
...
for sub_idx = 9   % subject '902'; Luis used subject '102' (sub_idx=1)
    ...
    fig_handle_sub = figure('Position', [100 100 400 1000]);
...
%         subplot(num_stage,1,s_idx)
        ...
        % allocate neg_corr_ratio and pos_corr_ratio into arrays
        neg_corr_array = [neg_corr_array; neg_corr_ratio];
        pos_corr_array = [pos_corr_array; pos_corr_ratio];
...
%% adjusting margins    
 ...
 %% save figure
    fig_name = 'barchart_C3_allsub_alltest_subject_902';
%     savefig(fig_handle_sub, [output_path, fig_name]);
%     saveas(fig_handle_sub,[output_path, fig_name],'epsc')
end