...
for FLAG_corr = 1:2
    ...
    % comment out if plot one channel
%     for s_idx=1:num_stage
...
%         for chn_idx = 1:num_channel
        for chn_idx = 3         % C3 channel
            ...
            link_width = 18*(abs(corr_ratio_avg(FLAG_corr,:)) + 1e-6); % thinker links if plot only one channel                                                          
...
%             panelplots(s_idx,row,col).select();   % plot all channels
            panelplots(s_idx).select();     % plot one channel at a stage
...
%                 plot(X(i),Y(i),'MarkerSize',50,'Marker','.','MarkerEdgeColor',labelcolor(s_idx,:));               
                plot(X(i),Y(i),'MarkerSize',125,'Marker','.','MarkerEdgeColor',labelcolor(s_idx,:));
...
%                 text(X(i)-0.1,Y(i)+0.05,freqbands{i},'FontSize',14);  
                ...
                % Larger size plots if plot one channel only
                % use same colormap for each stage to color-code freq nodes
                plot(X(i),Y(i),'MarkerSize',100,'Marker','.','MarkerEdgeColor',labelcolor(s_idx,:));               
                % add greek letters for each freq node
                text(X(i)-0.1,Y(i)+0.05,freqbands{i},'FontSize',30);  
    ...
    switch FLAG_corr
        case 1
            fig_name = 'C3_network_graphs_negative';
        case 2
            fig_name = 'C3_network_graphs_positive';
    end
...
for s_idx = 1: num_stage
    % load pairwise correlation for C3 channel
    ...
    pos_link_thickness_mat = 18*(abs(pos_degree_mat) + 1e-6);
    neg_link_thickness_mat = 18*(abs(neg_degree_mat) + 1e-6);
