function scatterbar3(X,Y,Z,width,labelcolor,histcolor)
...
for j=1:r
    ...
    for k=1:c
        if ~isnan(Z(j,k))
            fcolor = labelcolor(k,:);
            drawbar(X(j,k),Y(j,k),Z(j,k),width/2,fcolor,hcolor)
        end
    end
end
zlim=[min(Z(:)) max(Z(:))];
...
function drawbar(x,y,z,width,fcolor,hcolor)
    h(1)=patch([-width -width width width]+x,[-width width width -width]+y,[0 0 0 0],fcolor);  % bottom face
    h(2)=patch(width.*[-1 -1 1 1]+x,width.*[-1 -1 -1 -1]+y,z.*[0 1 1 0],fcolor);   % front face
    h(3)=patch(width.*[-1 -1 -1 -1]+x,width.*[-1 -1 1 1]+y,z.*[0 1 1 0],hcolor);   % left face
    h(4)=patch([-width -width width width]+x,[-width width width -width]+y,[z z z z],fcolor); % top face
    h(5)=patch(width.*[-1 -1 1 1]+x,width.*[1 1 1 1]+y,z.*[0 1 1 0],fcolor);   % back face
    h(6)=patch(width.*[1 1 1 1]+x,width.*[-1 -1 1 1]+y,z.*[0 1 1 0],hcolor);   % right face
    % set(h,'facecolor','flat','FaceVertexCData',z)
    % set(h,'facecolor',fcolor,'FaceVertexCData',z,'FaceAlpha',1)
end
